...
export const TalentTreeScroller = forwardRef<TalentTreeScrollerRef, TTalentTreeScrollerProps>(
  ({ trees }, ref) => {
    ...
    useEffect(() => {
      ...
      const handleScrollEnd = () => {
        ...
        if (scrollLeft < containerWidth * 0.2) {
          ...
          targetIndex = 0
        } else if (scrollLeft > (trees.length - 1) * containerWidth - containerWidth * 0.2) {
          ...
          const remainder = scrollLeft % containerWidth
          if (remainder < containerWidth * 0.3) {
            targetIndex = Math.floor(scrollLeft / containerWidth)
          } else if (remainder > containerWidth * 0.7) {
            targetIndex = Math.ceil(scrollLeft / containerWidth)
          }
        }
        ...
        if (Math.abs(scrollLeft - targetScrollLeft) > threshold) {
          container.scrollTo({
